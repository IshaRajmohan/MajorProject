...
class Observation(Base):
    ...
    raw_source_ref: Mapped[str] = mapped_column(String(255), nullable=True)
    status: Mapped[str] = mapped_column(String(20), default="pending")
    # NEVER update candidate_value/status of a past row in place — append new rows only.
...
class TwinState(Base):
    ...
    current_value: Mapped[dict] = mapped_column(JSON, nullable=True)
    confidence: Mapped[float] = mapped_column(Float, nullable=True)
    source_observation_id: Mapped[str] = mapped_column(String, nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    # ONLY the CAMS engine (via ObservationService) may write to this table.
...
class SyncDecision(Base):
    ...
    winning_observation_id: Mapped[str] = mapped_column(String, nullable=True)
    candidates_snapshot: Mapped[dict] = mapped_column(JSON)
    c1: Mapped[float] = mapped_column(Float)
    c2: Mapped[float] = mapped_column(Float, nullable=True)
